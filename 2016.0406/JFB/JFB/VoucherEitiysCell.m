...
@implementation VoucherEitiysCell
...
-(void)setHeight:(float)height{
    ...
    [self.contentView addSubview:_tradingParL];
    ...
    _payBtn.frame = CGRectMake(WITH*.76, height*.77, WITH*.2, height*.17) ;
    ...
    _logisticsBtn.frame = CGRectMake(WITH*.52, height*.77, WITH*.2, height*.17) ;
    
    //_payBtn.backgroundColor = [UIColor grayColor];饿疯似的饿死了；色；饿死了；DSL；来‘’‘’‘
